...
const topLevelInvestorInfo = (req, res) => {

    Model.BiddingModel.find({bidderId: req.user._id}).sort({"bidAmount": -1}).limit(1)
		.then(highestbid => {

            Model.BiddingModel.find({bidderId:req.user._id}).count()
			.then(totalbids => {
				console.log("res" , totalbids)

                Model.BiddingModel.find({bidderId:req.user._id,winner:true}).count()
                .then(approvedbids => {
                    console.log("res" , approvedbids)
                    ...
                    Model.BiddingModel.aggregate(
                        [
                            { "$match": { "bidderId": id }},
                            {$group: {"_id":"_id", avg_val:{$avg:"$bidAmount"}}}])
                            .then(events => {
                            console.log("eventttt" , events)
                            res.status(status.OK).send({highestBid:highestbid[0].bidAmount , totalBids:totalbids , approvedBids:approvedbids, averageBid: events[0].avg_val});
...
export default { getBiddings, bid , getUserBiddings , getSellerPropertyiesBiddings , getAllBiddingsOnProperty , editStatus , getHighestBidOnProperty , acceptBid , bidderCounts, topLevelInvestorInfo};